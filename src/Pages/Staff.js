...
function Staff() {
  ...
  const getDetails = (id) => {
    let item = staffs[id-1];
    setStaffName(item && item.staff_name);
    setStaffEmail(item && item.staff_email);
    setStaffPosition(item && item.staff_position);
    setStaffIdNumber(item && item.staff_id_number)
    setStaffMobile(item && item.staff_mobile);
  };
